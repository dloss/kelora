...
pub fn resolve_time_range(
    ...
    if until_uses_since_anchor {
        ...
        let since = if let Some(since_value) = since_str {
            Some(
                parse_timestamp_arg_with_timezone(since_value, default_timezone)
                    .map_err(|e| format!("Invalid --since timestamp '{}': {}", since_value, e))?,
            )
        } else {
            None
        };

        let until = if let Some(until_value) = until_str {
            Some(
                parse_anchored_timestamp(until_value, since, None, default_timezone)
                    .map_err(|e| format!("Invalid --until timestamp '{}': {}", until_value, e))?,
            )
        } else {
            None
        };
        ...
        let until = if let Some(until_value) = until_str {
            Some(
                parse_timestamp_arg_with_timezone(until_value, default_timezone)
                    .map_err(|e| format!("Invalid --until timestamp '{}': {}", until_value, e))?,
            )
        } else {
            None
        ...
        let since = if let Some(since_value) = since_str {
            Some(
                parse_anchored_timestamp(since_value, None, until, default_timezone)
                    .map_err(|e| format!("Invalid --since timestamp '{}': {}", since_value, e))?,
            )
        } else {
            None
