...
/// Resolve `--cut-at` to a single instant, accepting the same vocabulary as
...
/// user actually typed: routing `--cut-at` through the range resolver reported its
...
pub fn resolve_cut_timestamp(
    ...
    for anchor in ["since+", "since-", "until+", "until-"] {
        if let Some(offset) = arg.strip_prefix(anchor) {
            ...
            return Err(format!(
                "'{}' anchors a range bound, but --cut-at is a single instant with no range to anchor to. Give an absolute timestamp, or offset from now (e.g. now-{}).",
                bare, offset
            ));
        }
    }
