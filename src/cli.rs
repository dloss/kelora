...
pub enum DrainDiffFormat {
    #[default]
    Table,
    /// The same rows, each followed by indented detail: template id, the counts
    /// and shares the compact row leaves out, and a sample line.
    Full,
    Json,
    /// One tab-separated record per changed template, no header and no
    /// surrounding report, for piping to awk/sort/duckdb.
    Tsv,
    /// Resolve at output time: the diff report on a terminal, `tsv` when piped
    /// or redirected — the same rule `-m` and `--span-summary` follow. The
    /// default for a bare `--drain-diff`. Hidden because it is the implicit
    /// default rather than something to type.
    #[value(hide = true)]
    Auto,
}
...
pub struct Cli {
    ...
    #[arg(
        ...
        default_missing_value = "auto",
        help_heading = "Template Discovery",
        help = "Compare template frequencies between a baseline and a target log (summary-only; requires --keys with exactly one field; sequential mode only).\n\nThree ways to define baseline and target:\n  kelora --drain-diff old.log new.log -k msg      First input is baseline, second is target\n  kelora --drain-diff --cut-at 2026-07-24T14:00Z incident.log -k msg   One input split at a timestamp\n  kelora --drain-diff --cut-before 'e.msg.contains(\"deploy\")' incident.log -k msg   One input split at the first match\n\nDiffs *which message templates occur and how often*, not what the messages\nsay. Output reads like a unified diff:\n  --- / +++       name the baseline and target, with each side's event count\n                  and observed span (in the form --cut-at accepts back)\n  +               a template only the target has, with its event count\n  -               a template only the baseline had, with its event count\n  *               a template both have, at a materially different rate,\n                  shown as a plain multiple (\"14x more\")\n\nA * row is reported when the change is bigger than sampling noise for the\nnumber of events on each side, and big enough to act on (>= 0.5pp of share,\nor a >= 1.5x change in rate). Templates that change less are this diff's\ncontext lines: tallied in the footer as unchanged, not printed. All\ncomparisons use per-side shares, so sides of very different sizes compare\nfairly.\n\nRewording is invisible to it: \"timeout after 5s\" becoming \"timed out after\n5s\" reads as one - plus one +, never as a modified template.\nComposes with the pipeline: --filter/--exec run before diffing.\n\nA vacuous comparison is refused rather than reported (exit 1):\n  - No event carries the field named by -k: the error names the nearest field it\n    did see, instead of printing an all-empty \"nothing changed\" report.\n  - One side got every event and the other none (usually a splitter that missed):\n    every template would be + or - by construction, so the error names the\n    resolved cut and the span the input covers, or says the predicate never\n    matched, instead.\nA field present on only some events warns with the excluded and compared counts.\n\nFormats:\n  table   Diff-style rows + footer. The default on a terminal.\n  full    The same rows, each followed by indented detail: template id,\n          both counts and shares, and a sample of a real line it matched.\n  tsv     One 'change<TAB>template_id<TAB>counts<TAB>percents<TAB>template'\n          record per changed template; no header, no report around it.\n          The default when stdout is piped or redirected (like -m), so\n          --drain-diff=table forces the report through a pipe.\n  json    One JSON object (new/gone/freq_changed, per-side totals, the\n          unchanged tally, spans and exclusion counts; each freq_changed\n          entry carries delta_pp and a signed z_score for your own\n          filtering)\n\nExamples:\n  --drain-diff old.log new.log -k msg\n  --drain-diff=json before.log after.log -k msg\n  --drain-diff --cut-at 2026-07-24T14:00Z incident.log -k msg\n  --drain-diff --cut-before 'e.msg.contains(\"deploy started\")' incident.log -k msg"
    )]
    ...
    #[arg(
        ...
        value_name = "TIME",
        help_heading = "Template Discovery",
        help = "Timestamp splitting one --drain-diff input into baseline (before the cut) and target (at/after).\n\nAccepts the same timestamp formats as --since/--until (e.g., 2026-07-24T14:00:00Z, '2026-07-24 14:00', '1h', 'yesterday'). See --help-time.\nEvents without a parseable timestamp are excluded from the comparison and counted in a warning.\n\nThe cut resolves against the clock, not the log: '1h' means an hour before now,\nand a bare '14:00' means today at 14:00. For a historical log, give an absolute\ntimestamp — a cut that lands outside the data leaves one side empty, which is\nrefused (exit 1) with the span the input actually covers, so a missed first\nattempt tells you what to use. When you know what the change looks like but not\nwhen it happened, --cut-before / --cut-after split on a predicate instead and\nneed no clock.\n\nEvery report states where the split landed ('baseline spans .. / target spans ..'),\nin the same form --cut-at accepts back.\n\nExample:\n  --drain-diff --cut-at 2026-07-24T14:00Z incident.log -k msg"
    )]
    pub cut_at: Option<String>,

    /// Rhai predicate whose first match splits a single --drain-diff input; the matching event starts the target.
    #[arg(
        long = "cut-before",
        value_name = "EXPR",
        help_heading = "Template Discovery",
        conflicts_with_all = ["cut_at", "cut_after"],
        help = "Boolean Rhai expression whose first match splits one --drain-diff input, with the cut placed immediately BEFORE the match: the matching event is the first target event.\n\nUse this when the matching line is the start of the new regime — a deploy\nmarker, the first error of an incident. Use --cut-after instead when the match\nis the end of the old one.\n\nNeeds no timestamps and no clock: you say what the change looks like, not when\nit happened. The boundary latches on the first match, so a predicate that keeps\nmatching still splits the log exactly once, and no events are buffered.\n\nSame expression syntax as --filter (see --help-rhai), and -I/--include helpers\nare available. A predicate that never matches leaves the target side empty, and\none matching the first event leaves the baseline empty; both are refused (exit 1)\nrather than reported as change. One matching the last event leaves a one-event\ntarget, which prints with a warning (🔸) rather than being refused.\n\nExamples:\n  --drain-diff --cut-before 'e.msg.contains(\"deploy started\")' incident.log -k msg\n  --drain-diff --cut-before 'e.status >= 500' access.log -k msg\n  --drain-diff --cut-before 'e.version != \"1.4.2\"' rollout.log -k msg"
    )]
    pub cut_before: Option<String>,

    /// Rhai predicate whose first match splits a single --drain-diff input; the matching event ends the baseline.
    #[arg(
        long = "cut-after",
        value_name = "EXPR",
        help_heading = "Template Discovery",
        conflicts_with_all = ["cut_at", "cut_before"],
        help = "Boolean Rhai expression whose first match splits one --drain-diff input, with the cut placed immediately AFTER the match: the matching event is the last baseline event.\n\nUse this when the matching line closes out the old regime — 'deploy finished',\n'migration complete', the last healthy heartbeat. Use --cut-before when the match\nopens the new one.\n\nOtherwise identical to --cut-before: same expression syntax as --filter (see\n--help-rhai), -I/--include helpers available, boundary latches on the first\nmatch, nothing buffered. A predicate that never matches, or one matching the last\nevent, leaves the target side empty and is refused (exit 1) rather than reported\nas change. One matching the first event leaves a one-event baseline, which is\nwell-formed enough to report: it prints, with a warning (🔸) that the NEW section\nreflects that shortfall rather than a change.\n\nExamples:\n  --drain-diff --cut-after 'e.msg.contains(\"deploy finished\")' incident.log -k msg\n  --drain-diff --cut-after 'e.phase == \"warmup\"' rollout.log -k msg"
    )]
...
/// Load every `-I/--include` file, ignoring the positional mapping that scopes
/// includes to individual script stages. Used by `--cut-before`/`--cut-after`, a single
/// global predicate rather than one of the ordered stages.
pub fn load_all_include_files(includes: &[String]) -> Result<Vec<crate::config::IncludeFile>> {
    load_include_files(includes)
}
