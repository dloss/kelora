//! Tailmap output formatter
//!
//! Visualizes numeric field distribution focused on tail latencies.
...
use super::compact_map::{extract_timestamp, format_line};
    ...
    fn finish(&self) -> Option<String> {
        ...
        for entry in &state.entries {
            ...
            if visible_len >= available_width {
                ...
                output.push_str(&format_line(current_timestamp.as_ref(), &buffer));
                buffer.clear();
                visible_len = 0;
                current_timestamp = Some(entry.timestamp.clone());
            }
