...
- **A metric recorded by an event whose script then failed is no longer discarded** - `--exec 'track_freq("lvl", e.level); if bad { throw "…" }'` ran the `track_freq` call and then lost that event's contribution: the failing event's metrics never left the script stage, and the next event's arrival overwrote them. The call ran, so it now counts.

  This looked like "a failed event doesn't count", but it was never that rule. It depended on **where the event sat in the file** — a script that threw on the *last* event kept its contribution, because there was no next event to overwrite it, while the same failure one line earlier lost it. It was scoped to **one stage, not the event** — with two `--exec` stages and a throw in the second, the first stage's count for that event survived and the second's did not. And it disagreed with the **events kelora actually emitted**: an exec error is non-fatal, so the event is still written to stdout, leaving three events out and two counted. Tracking's rule everywhere else is that a `track_*` call counts when it executes, whatever becomes of the event afterwards — a metric recorded before a later `--filter` drops the event still counts it. The error path was the one place that rule didn't hold.

  What changes for a script that both tracks and throws: the totals grow by the events that failed after the call. Where that isn't wanted, moving the risky part *above* the `track_*` call gives the old numbers back, exactly and by position rather than by luck — which the previous behavior offered no way to express in reverse. Only events whose script errors are affected: a run with no script errors reports exactly what it did before, sequential and `--parallel` agree as they did, and error counts, exit codes and `--strict` are unchanged.
