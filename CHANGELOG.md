...
- **Auto-detection now samples the file head and handles mixed-format files** - `-f auto` (the default) and `-f auto-per-file` used to judge a whole file by its first non-empty line: a stray banner line pinned everything to `line`, and a JSON log with interleaved plain-text lines dropped those lines as parse errors, with a post-run warning telling you to re-run with `-f json,line`. For **file input**, detection now samples up to the first 64 non-empty lines (capped at 256 KiB). A homogeneous sample detects exactly as before; a mixed sample makes kelora do what that warning suggested, automatically — parse with a cascade of the detected formats (specific first, `line` last), adding the usual `_format` field per event. Under `-v` the decision is stated as `Auto-detected format: cascade(json,line) (mixed formats in first N lines)`. CSV/TSV stays whole-file: a file starting as csv/tsv never becomes a cascade, and a csv-looking line mid-file counts as `line`. **stdin is unchanged** — a live pipe never waits for a sample, so mixed streams still want an explicit `-f json,line`.
