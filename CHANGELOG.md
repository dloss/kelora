...
- **`--drain-diff` caps each marker group at 20 rows** - A high-cardinality field produces a long run of `+` rows however good the clustering is, and past a screenful the ranking the sort exists to provide is lost. The rest collapse into one line naming how many templates and events were held back, and pointing at `--drain-diff=json` for all of them. Never a silent cut, and never applied to the machine formats: `tsv` and `json` carry every row, because a record stream with rows silently missing is worse than a long one.
