...
- **`-A`/`-B`/`-C` context: every event now appears once, with the right marker** - The four markers were being computed correctly and then applied to the wrong number of lines. The before-context loop re-walked its whole ring buffer on every match with no record of what it had already emitted, so a line covered by two matches was printed **twice** — first as after-context (`\`), then again as overlap (`|`) — and a match that fell inside another match's before-window was printed once per window it landed in. Three consecutive matches with `-C 2` produced 11 lines for 7 events. These were duplicated *events*, not just repeated markers: `-F json` emitted the same object twice, and `-s` reported `12 total, 6 output, 8 filtered` for a 12-event run. Reading only a duplicated line's first copy showed `\` where `|` belonged, which is what made the markers look arbitrary. The cause is structural — whether an after-context line is *also* the run-up to a later match cannot be known until up to `before_context` further events arrive — so context lines are now held back that far before being marked and released, which is what makes a single correct marker possible; the queue is bounded by `-B`'s value and drains at end of input. Marker semantics are unchanged and now match `grep`'s model exactly.

- **`--levels`/`--exclude-levels` with `-A`/`-B`/`-C` showed no context at all** - The level filter carried its own copy of the context code with the central test inverted (`if !buffered.is_match { continue }`), so it skipped precisely the non-matching lines that *are* the context and re-emitted only previously-emitted matches; its after-context branch counted the window down and then discarded the line. `kelora -j -l error -C 1` printed the error events and nothing else — indistinguishable from omitting `-C`, with no warning — while the equivalent `--filter 'e.level == "error"' -C 1` worked. The context code now exists once and both filters use it, so `--levels` and the equivalent `--filter` produce identical output. Note that context lines around an `--exclude-levels` match are frequently *of* an excluded level: that is what the neighbouring lines are, and suppressing them is what left `-B`/`-A`/`-C` with nothing to show.

- **`--filter` combined with `--levels`, or repeated, now gets one context window instead of none or several** - Context was handed to *every* `--filter` stage, stacking one buffer per stage: `--filter A --filter B -C 1` emitted the match three times (twice from stage one). Meanwhile the level filter only received a context config when no `--filter` existed at all, so `--filter … --levels error -C 1` ran the level stage context-free downstream of the filter and stripped the context lines the filter had just selected — back to matches only. Context is now resolved once, by a single stage wrapping the pipeline's leading run of filters, against their **combined** verdict: `--filter A --filter B -C 2` and `--filter A -l error -C 2` both mean "context around events passing both", which is what they read like. `--since`/`--until` stay outside that run — they define which events exist at all, so context is drawn from inside the time window rather than across its edges — and a filter separated from the run by an `--exec`/`--assert` also stays outside, since folding it in would move it ahead of that stage; because such a filter can drop matches and leave their context behind, kelora now warns (🔸) when one is there.

- **Context markers no longer disappear under `--no-diagnostics`** - The markers are part of the event line on stdout, but they were gated on the derived `quiet_level`, which `--no-diagnostics` (and `--no-warnings --no-hints`, and `KELORA_NO_WARNINGS`+`KELORA_NO_HINTS`) raises to 1. So the flags for quieting *stderr* silently stripped `/`, `\`, `|` and `*` from the data, leaving a leading space where a marker belonged — a plausible reading of "the symbols are sometimes wrong". The gate was also vestigial for the case it named: `-q`/`--silent` suppress events by swapping in a different formatter, so nothing reaches this code path there. Removed; `-q` and `--silent` are unaffected.

  Also fixed in the docs: `docs/reference/cli-reference.md` described `|` as marking "separator lines" — it marks a line that is before-context and after-context at once. `-A`/`-B`/`-C` now document the marker set, what counts as a match, and the sequential-mode requirement in `--help` as well.

- **`--drain-diff` no longer presents a whole log as changed when the split left one event on a side** - A side with *no* events was already refused (exit 1), because every template lands on a `+` or `-` row by construction and the report reads as a dramatic finding when it only means the boundary missed. A side with *one* event slipped through that test and produced the same misleading report at exit `0` with nothing said: `--cut-before` whose marker matches the last event of the capture — the likely real mistake, "I grabbed the log too early" — reported every message type as removed, and `--cut-after` matching the first event reported the whole log as added. Both the two-input form (a target file with one line) and a `--cut-at` landing one event inside the range did the same. Such a report is *well-formed* — shares and z-scores compute — so it is now warned about (🔸) rather than refused, matching how the other compromised-corpus cases (cluster-cap eviction, partial field exclusion, zero compared events) are already handled; the empty-side refusals are unchanged. The trigger is derived rather than tuned: a side that contributed fewer events than the other side has distinct templates cannot have exhibited that variety, so the number of rows is bounded by the sample instead of by any change. It deliberately stays quiet for a homogeneous-but-well-sampled side — 500 events of one template against five templates is a real finding, not an artifact. `--cut-after`'s help claimed "a degenerate split is refused (exit 1)", which was never true for the one case only `--cut-after` can reach; both predicate flags now state the empty-side refusal and the one-event warning separately.
...
- **`--drain` groups lines it had been splitting, and no longer needs a vendored fork to do it** (see also the two masking/generalization entries below) - Template *text* was fixed earlier on this branch; which lines ended up together was not, and that turned out to be the bigger problem. The vendored `drain-rs` keyed its tree on `max_depth` leading tokens *and* on the token at the final level, terminating at `depth == len - 1`. So every token of a message no longer than `max_depth + 1` was part of the routing key — and a message whose every token is a key can never generalize, because no position is left for the algorithm's defining step. No setting could work around it: `Invalid user <user> from <ip>` mined 102-110 separate templates at every `depth`. The same accounting made `depth` mean something other than the literature's `depth` (the reference implementation subtracts 2 and starts at 1, so its default of 4 keys on *one* token, where kelora's identical-looking 4 keyed on five), which split every message varying in an early position: `Failed password for <user> from <ip> port <n> ssh2` became one template per user name. The tree is now kelora's own (`src/drain/tree.rs`; `vendor/drain-rs` is gone), `depth` means the number of keyed tokens, and it is never more than one below the message's length. Because kelora masks before clustering, the tree also *knows* which positions hold values instead of guessing from "does this token contain a digit" — a token that is nothing but a placeholder keys as a wildcard, while `uid=<num>` and `HTTP/<version>` key on the literal text that names the message. On the loghub Linux sample, `--drain -k msg` goes from 123 templates to 115: `session closed for user <*>` over all 123 events, `connection from <ipv4> <*> at <timestamp>` over all 909 where four templates used to disagree. OpenSSH goes from 129 templates to 28, against 27 real message types. Expect templates and template IDs to change for anything that was fragmented.

- **Templates that describe one message are merged once the whole set is visible** - The tree has to place a line the moment it arrives, from a keyed prefix and whatever clusters exist at that point, which fragments in a predictable shape: a family of templates identical but for a single position. At end of input that constraint is gone — a few hundred templates, the whole family visible at once — so `--drain` and `--drain-diff` now merge such a family when the evidence says the position holds a value: either some member already generalized it to `<*>` (the tree's own conclusion, reached late for the rest), or a dozen-plus distinct values appear there across otherwise-identical messages. A keyword does not look like that; `session opened` versus `session closed` is a family of two. Any merge that would leave a template with no literal text of its own is refused, which is what keeps distinct event names (`onStandStepChanged <num>`, `onReceive <num>`, …) from collapsing into a meaningless `<*> <num>`, and stops `<*> <path> <num>` swallowing every HTTP method. This is what `--drain-diff` needed most: a brute-force burst that reported as **85 separate `+` rows and no rate change at all** now reports one row, `Invalid user <*> from <ipv4>`, `6.4× more`.

- **`--drain` clustering is now measured, and the defaults come from the measurements** - The two `--drain` fixes shipped earlier on this branch rewrote every template's text while moving grouping accuracy by 1.6pp, and nothing in the repo could show either fact. `just drain-accuracy` scores all 16 [loghub](https://github.com/logpai/loghub) `_2k` datasets (2000 human-annotated lines each) against their ground truth and fails against a committed baseline; the corpus is fetched on demand rather than vendored, and like fuzzing it stays out of CI. It reports four numbers per dataset, because accuracy alone hides which way the errors run and rewards the wrong fix — over-splitting is noise a reader can see through, while over-merging destroys a distinction that cannot be recovered from the output, so a change that raises accuracy by merging more aggressively fails the gate. Mean grouping accuracy across the suite went from 73.0% to 83.7% over the changes above, with fewer events in over-merged templates than before (1742, from 2049) and no dataset worse than it started. The `depth` and `similarity` defaults (now 2 and 0.8) were picked from a grid over that suite rather than copied from the paper — at the paper's 0.4 the new tree scored *worse* than what it replaced, because with fewer keyed tokens the similarity bar is what keeps unrelated messages apart.

- **`--drain` refuses a mined field that was never present, instead of printing nothing** - `--drain -k message` on a log whose field is `msg` printed no templates and exited 0, applying the rule of silence to what is really a usage error. `--drain-diff` already refused the same mistake and named the fields it had seen; the two modes now agree. Partial exclusion stays silent (a template list carries no totals, so it never implies it covered every event), and an empty input still exits 0 with no output.

- **`--drain` no longer grows without bound on an endless stream** - The template model had no cap, so `tail -f … --drain` on a high-cardinality field grew until the process died. It now holds 10,000 templates and evicts the least-recently-matched one beyond that, with a warning (🔸) that counts are incomplete and that the fix is to normalize the field.
...
- **`--drain-diff` — template-level log comparison** - Answer "what changed?" between a baseline and a target log, and answer it as a diff. It diffs *which message templates occur and how often*, not what the messages say. Three ways to define the sides: two inputs (`kelora --drain-diff old.log new.log -k msg`, first is baseline), one input split at a timestamp (`--drain-diff --cut-at 2026-07-24T14:00Z incident.log -k msg`, everything before the cut is baseline), or one input split at the first match of a predicate (`--cut-before` / `--cut-after`, no timestamps needed). The report borrows unified-diff syntax, so the shape is readable before the semantics are: `---` and `+++` name the two sides with each side's event count and observed span (in the form `--cut-at` accepts back, so a first attempt that misses doubles as the lookup for the next), then one row per changed template — `+` a template only the target has, `-` one only the baseline had, `*` one both logs have at a materially different rate, annotated with that rate as a plain multiple (`14× more`). Rewording is invisible to it by construction: "timeout after 5s" becoming "timed out after 5s" reads as one `-` plus one `+`, never as a modified template. Both sides are mined jointly, then counted against the frozen final template set, so results are deterministic and swapping the inputs only flips the sign of the deltas. Comparisons use per-side *shares* rather than raw counts, so a 10-minute incident capture diffs fairly against a 24-hour baseline, and the multiple on a `*` row is computed from shares so it stays honest when the sides are different sizes (raw counts, 30 vs 2, would claim 15× where the shares say 14×). There are deliberately no threshold flags. A `*` row is reported when the change is too big to be chance at those event counts *and* big enough to matter (≥ 0.5 points of the side's traffic, or a ≥ 1.5× change in rate) — so a single event on a 20-event side stays out while a sub-percent move backed by thousands of events gets through, and changes that are real but trivially small don't take up a row. Everything else is this diff's *context lines*: templates whose frequency did not meaningfully change, tallied in the footer rather than printed. If one of those did move by a point of traffic or more the footer says how many and at what event counts ("2 of them changed a little, but 110 and 120 events are too few to tell that from random variation"), so a held-back drop never looks like an omission. Rows are sorted by how much of the side's traffic moved so the eye does the thresholding, and `+` templates are reported down to a single occurrence. Templates are truncated to the terminal width rather than wrapped, so the marker and annotation columns stay aligned; `COLUMNS` is honored even when redirected, and `--no-emoji` swaps the Unicode punctuation for ASCII. Four formats, and a bare `--drain-diff` picks between two of them the way `-m` and `--span-summary` do: `table` (the report, on a terminal), `tsv` (one fixed nine-column `change<TAB>template_id<TAB>counts<TAB>percents<TAB>template` record per changed template, no header and no report around it — the default when stdout is piped or redirected, so `--drain-diff=table` forces the report through a pipe), and `json` (one object with `new` / `gone` / `freq_changed`, per-side totals and spans, the unchanged tally, and the exclusion counts; each `freq_changed` entry carries `delta_pp` for the share move and a signed `z_score` for downstream filtering at your own cutoff). `full` mirrors `--drain=full`: the same rows, each followed by the template id, both counts and shares — so a `*` row's multiple can be checked against the shares it came from — and a **sample of a real line** the template matched, which is the first thing anyone wants after reading a template. The sample is the most frequent line on that side, tie-broken on the text, so it is representative and identical between two runs over the same log; `json` carries the same samples per side, `tsv` does not, because widening its record to hold free-form log text would cost the fixed column count that makes `awk` positions reliable. One vocabulary across all four: the `tsv` kind column and the JSON array names match the table's `+`, `-` and `*`, and every JSON count, percent and sample names the side it belongs to. Composes with the pipeline: `--filter`/`--exec` run before the comparison (diff only errors, or normalize custom tokens first). Requires `--keys` with exactly one field, like `--drain`; sequential mode only. An all-empty report reads as a confident "nothing changed", so kelora refuses to print one when the mined field never yielded a value: a `-k` naming a field no event carries fails with exit `1` and suggests the nearest field it did see, rather than certifying a log as unchanged that was never examined. Softer cases stay non-fatal — a field present on only *some* events warns with the excluded and compared counts (`excluded_no_field` in the JSON form), and a run where nothing reached the comparison at all still prints the report, warning that it reflects missing data rather than an unchanged log.
