...
- **Integer overloads for `to_datetime()`** - `to_datetime()` now accepts an integer Unix timestamp directly, so numeric epoch fields from JSON logs no longer need a `.to_string()` round-trip. The bare `to_datetime(int)` form infers the unit from the digit count exactly like the existing string path (10→seconds, 13→ms, 16→µs, 19→ns), so `to_datetime(1700000000) == to_datetime("1700000000")` and the same digit counts that error as strings also error as integers — no new heuristic, and no change to string or formatter behavior. Because that inference is silent and leaves dead zones (ambiguous digit counts, and small values such as the epoch `0`), a second `to_datetime(int, unit)` overload takes an explicit unit (`"s"`/`"ms"`/`"us"`/`"ns"`) for unambiguous conversion: `to_datetime(e.ts, "ms")`, `to_datetime(0, "s")`. The ambiguous bare-integer failure now carries an actionable message pointing at the explicit-unit form.
