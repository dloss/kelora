...
- **Empty `--metrics` explains missing-field skips** - When `-m`/`--metrics` produces nothing because every `track_*` value was a missing field (the common field-name-typo case), the output now reads `No metrics tracked: every value was missing for <name> (<count>). A track_* call likely references a field that doesn't exist — run --discover to list the fields actually present.` instead of a bare `No metrics tracked`. Unlike the broader `--diagnostics` skip line, this surfaces unconditionally, because an empty result is exactly the moment a stuck user needs the hint. A genuinely empty run (no `track_*` calls, or none that skipped) still shows the terse message.
