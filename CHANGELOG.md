...
- **A failing script stage now takes its `track_*` metrics back reliably, instead of by luck of position** - Script stages are atomic: a stage that errors falls back to the state it had before it ran, so the event keeps its old fields, `emit_each` output is dropped and queued file writes never happen. Metrics were supposed to be part of that fallback and mostly looked like they were, but the mechanism was accidental — the failing event's metrics simply never left the stage, and the *next event's arrival* overwrote them. So the rollback depended on there being a next event: `--exec 'track_freq("lvl", e.level); if bad { throw "…" }'` threw away the failing event's count everywhere in the file except on the **last event**, where the count survived into the report. Under `--parallel` the boundary was the batch rather than the file, so the same log could report either number depending on `--batch-size`.

  Each stage now undoes exactly what it recorded, so the outcome no longer depends on where the event sits, how the input is batched, or whether anything follows it. `track_freq`, `track_sum`, `track_inc`, `track_avg`, `track_min`, `track_max`, `track_unique`, `track_cardinality`, `track_percentiles`, `track_stats` and the four ranking functions all roll back, and a metric whose only value is undone disappears rather than remaining as an empty table. Error counts, gate counters and skip tallies are deliberately **not** rolled back — they exist to record that the failure happened — so exit codes, `Exec errors: N total` and `--strict` are unchanged.

  Stages remain independent transactions, which is also how to opt out: a `track_*` call in its *own earlier* `--exec` stage commits when that stage completes, whatever a later stage does with the event.
