...
- **The zero-match hint no longer reports a method name as a missing field** - `kelora -j app.jsonl --filter 'e.has("nope")'` answered `0 events matched. Filter referenced unseen field: has.` — `has` is a method, one kelora itself recommends, and there was never going to be a field by that name. The hint collected candidate fields by scanning identifiers after `e.` and could not tell `e.status` (a field read) from `e.has(…)` (a call), so it fired on a *correct* expression whose zero matches were the true answer, and sent you hunting a typo in the one identifier you were sure about. A call is now recognised as a call and names no field. `e.has("field")` goes one better: the string argument *is* the field under test, so the hint names that instead — `unseen field: nope` — which makes it right rather than merely quiet. Dotted literals are deliberately left alone: `e.has("http.status")` is a nested path, and reporting it as never present would be the same false claim in a new place. (#365)

- **`--freq`, `--describe` and `--card` accept a comma-separated list** - `-k`, `-l` and `-f` all take comma lists, so `--card user_id,customer,trace_id` is the natural thing to type. It was accepted as a *single* literal field name, matched nothing, and produced `Tracking skipped events with missing values: user_id,customer,trace_id (2). These metrics never recorded a value — likely a field-name typo.` — the opposite of the truth, since every field existed and `--freq` on any one of them counted fine. The run also emitted only that hint and no metrics at all, so a pipeline with stderr redirected got an empty result and no reason. All three flags now split on commas like their siblings; repeating the flag works exactly as before, and `--freq a,b` is equivalent to `--freq a --freq b`. Note the consequence: a top-level field whose literal name *contains* a comma is no longer reachable through these flags, the same trade `-k` has always made. An expression typed into a FIELD slot still gets the hint that names `track_freq` rather than a typo report. (#366)

- **The unknown-variable hint now names `state` and `metrics`** - A script reaching for an accumulator got `Hint: Available variables: e (event), meta (metadata), conf (initialization data), line (raw line)` — a hardcoded list omitting the only two variables that can help someone who just hit "Variable not found", since hitting it almost always means wanting cross-event state. Worse, it named `conf`, which is read-only outside `--begin`, so taking the hint led straight to `conf map is read-only outside --begin` — a second error closing the door the first had pointed at, with neither message saying the word `state`. Only the *fuzzy* branch knew better: it reads the live scope, so `let t` got `Did you mean: meta, state, metrics?` while realistic names like `tot` and `hourly_totals` fell through to the incomplete list. The fallback now reads that same scope, so the two branches cannot disagree, and each variable carries the caveat that decides whether reaching for it can work (`state` — sequential only; `conf` — read-only after `--begin`). Because it reflects the failing stage, a per-event stage no longer lists `metrics`, which genuinely is not readable there, while `--end` does; `--span-close` gains `span`. The immutability error also names the alternative now: *"Use `state["key"]` for mutable cross-event state (sequential mode only)"*. (#360)
