...
- **Cascade mode no longer overwrites a `_format` field the log already had** - The winning parser name was written with a blind insert, so a record carrying its own field named `_format` — an ECS log with `"_format":"ecs-1.6"`, a pipeline that tags its own records — had that value replaced by `"json"` on every event, with no warning and exit `0`. The only trace was under `-v`. This was reachable since 1.5.0 but needed an explicit `-f json,line`, so the user had opted into cascade parsing; with head sampling and mid-file probes (above) a plain `-f auto` can now select a cascade by itself, which widened the trigger to roughly any plain file ≥32 KiB whose format varies anywhere in it. kelora now keeps your value and skips the tag for that event: the parser name is recoverable from `--stats` (`Cascade formats: …`), your log line is not. A warning reports how many events were affected, so the missing tag is explained rather than mysterious, and it names the single explicit `-f FORMAT` that avoids the cascade altogether. Events with no `_format` of their own are tagged exactly as before, the per-format counts in `--stats` still cover every line regardless of tagging, and the exit code stays `0` — nothing was lost. (#406)
