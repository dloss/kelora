...
- **`--drain-diff` no longer presents a whole log as changed when the split left one event on a side** - A side with *no* events was already refused (exit 1), because every template lands in NEW or VANISHED by construction and the sections read as a dramatic finding when they only mean the boundary missed. A side with *one* event slipped through that test and produced the same misleading report at exit `0` with nothing said: `--cut-before` whose marker matches the last event of the capture — the likely real mistake, "I grabbed the log too early" — reported every message type as VANISHED, and `--cut-after` matching the first event reported the whole log as NEW. Both the two-input form (a target file with one line) and a `--cut-at` landing one event inside the range did the same. Such a report is *well-formed* — shares and z-scores compute — so it is now warned about (🔸) rather than refused, matching how the other compromised-corpus cases (cluster-cap eviction, partial field exclusion, zero compared events) are already handled; the empty-side refusals are unchanged. The trigger is derived rather than tuned: a side that contributed fewer events than the other side has distinct templates cannot have exhibited that variety, so the section length is bounded by the sample instead of by any change. It deliberately stays quiet for a homogeneous-but-well-sampled side — 500 events of one template against five templates is a real finding, not an artifact. `--cut-after`'s help claimed "a degenerate split is refused (exit 1)", which was never true for the one case only `--cut-after` can reach; both predicate flags now state the empty-side refusal and the one-event warning separately. The totals line also said `target 1 events`; it now says `1 event`.
...
- **`--drain-diff` sections are capped at 20 rows** - A high-cardinality field produces a long `NEW` section however good the clustering is, and past a screenful the ranking the sort exists to provide is lost. The rest collapse into a footer naming how many templates and events were held back, and pointing at `--drain-diff=json` for all of them. Never a silent cut.
