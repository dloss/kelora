...
- **`track_min`/`track_max` on float values no longer report one partition's extreme under `--parallel`** - `kelora -P -m --exec 'track_max("mx", e.latency_ms)'` returned the maximum of whichever worker's batch happened to merge last, not the maximum of the file — wrong in 17 of 20 runs on a 4000-event log (reporting `186.39` where the true maximum was `2914.06`), and wrong nondeterministically, so a second run could look fine. Nothing marked the result: exit `0`, no warning, no hint, even with `--warnings --hints --verbose`. Both mergers read each side with `as_int()`, which fails on a float, and a merger that declines to merge falls through to replacing the stored value — so for float metrics the merge step was effectively last-writer-wins. Integer metrics merged correctly throughout, and `min` often looked right by coincidence, since a batch containing the overall minimum makes any winner the correct one. The reach was every consumer of those two aggregators, including `track_stats` and `--describe`, whose `_min`/`_max` rows disagreed with the same command run sequentially; additive metrics (`track_sum`, `track_inc`, `track_avg`, `track_freq`) and percentiles were never affected. Both mergers now compare in `f64` space the way the sequential path does, so int and float batches of one metric combine, and return the winning side's value **unconverted** — an int metric stays an int under `-P` rather than becoming `504.0` — with ties and `NaN` resolving as they do sequentially. `-P` and sequential now agree on `_min`/`_max` for float and mixed-type inputs alike.
...
- **An unknown `absorb_*` option is no longer a silent no-op** (#364) - `e.absorb_regex("a", "x=(?P<x>\d+)", #{types: true})` extracted nothing and said nothing: the whole call was discarded over the third argument, and the run exited `0` with the events untouched. The diagnostic was never missing — `unknown absorb option: types` was right there in the returned status map, reachable only by assigning the return value and printing it, which no working script does. An option typo is a script bug, so it now goes where script bugs go: `--exec` rolls the event back and reports `Exec errors: N total, affecting every event` on stderr (still exit `0`, since transforms are best-effort), and `--strict` aborts on the first one, as it already did. The message also names the keys that would have worked (`valid: sep, kv_sep, keep_source, overwrite`), so the fix doesn't need a second command. A **pattern that does not compile** is the same class of bug — no input can make an invalid regex match — and is raised the same way, with the regex crate's four-line error condensed to the one line that says what's wrong. Everything *data-dependent* stays as quiet as before: a pattern that simply didn't match this line (`empty`), an absent field (`missing_field`), a non-string field (`not_string`), and malformed content in `absorb_json`/`absorb_logfmt`/`absorb_jwt` (`parse_error`) are all normal for a real log and still come back only in the status map. Scripts that branch on `status == "invalid_option"`, or on `"parse_error"` from an uncompilable pattern, no longer see those values without a `try`/`catch`.
